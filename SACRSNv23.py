# ============================================================
# SACRSN v19: STABILIZED OMNI-SCIENTIFIC EDITION
# Features: True Stack Persistence, Safe Autograd, Correct Decoding
# ============================================================
...
class UberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, prev_sym=None):
        ...
        # [FIX] Hidden State Logic: True Recurrence
        if hidden is None:
            # Init Latent
        ...
        act_step = 0
        halting_prob = torch.zeros(batch_size, 1).to(z.device)
        ...
        for t in range(CONFIG["max_recursion_depth"]):
            ...
            if CONFIG["use_stack"]:
                ...
                stack_history.append(depth_idx.item())
            else:
                z_combined = z_proc
                stack_history.append(0.0)
            ...
            still_run = (halting_prob < CONFIG["act_threshold"]).float()
            p = p_halt * still_run
...
def train():
    ...
    print(f"--- Training SACRSN v19 (Stabilized) ---")
        ...
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(0, len(data_tensor) - 1):
                ...
                loss_temporal = CONFIG["symbol_consistency_weight"] * F.mse_loss(
                    curr_onehot, 
                    model.prev_sym_soft.detach() # Compare against fixed target
                )
...
def visualize_all(model):
    ...
    for _ in range(200):
        with torch.no_grad():
            ...
            if s_logs:
                # Average recursion depth for this token
                stack_history.append(np.mean(s_logs))
            else:
                stack_history.append(0)
            
            act_history.append(1.0 + ponder.item())
            
            # Extract z from hidden state tuple (z, mem, ptr)
